...
CompassAlpha is young, and its governance reflects that. During the v0.x line, decision-making is intentionally simple and centralized so the doctrine can converge quickly. This page describes the current model and how it is expected to evolve.
...
Changes to the framework's doctrine — axioms, guardrails, tunables, toggles — do not land ad hoc. They travel through the framework's own **doctrine-cycle discipline**: a change is proposed, captured on disk with its rationale and substrate citation, reviewed against the existing axioms for consistency, and only then locked in. Invariant axioms carry the highest bar. See [Contributing](contributing.md) for the contributor-facing view of this process.
